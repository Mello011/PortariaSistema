/**
 * PontoPortaria - Application Logic & Supabase API Integration
 */
...
const mockStore = {
    jornadas: [
        { id: 1, cargo: 'PORTEIRO', horario_entrada: '07:00', horario_saida: '19:00', tolerancia_entrada_minutos: 5, tolerancia_saida_minutos: 10, exige_fechamento_chaves: false },
        { id: 2, cargo: 'VIGILANTE', horario_entrada: '19:00', horario_saida: '07:00', tolerancia_entrada_minutos: 10, tolerancia_saida_minutos: 30, exige_fechamento_chaves: false },
        { id: 3, cargo: 'ZELADOR', horario_entrada: '08:00', horario_saida: '17:00', tolerancia_entrada_minutos: 10, tolerancia_saida_minutos: 10, exige_fechamento_chaves: true },
        { id: 4, cargo: 'RECEPCIONISTA', horario_entrada: '09:00', horario_saida: '18:00', tolerancia_entrada_minutos: 10, tolerancia_saida_minutos: 10, exige_fechamento_chaves: true }
    ],
    funcionarios: [
        { id: 1, matricula: 'FUNC001', nome: 'Mário Santos', cpf: '11122233344', jornada_id: 1, ativo: true },
        { id: 2, matricula: 'FUNC002', nome: 'Carlos Andrade', cpf: '22233344455', jornada_id: 2, ativo: true },
        { id: 3, matricula: 'FUNC003', nome: 'Ana Ferreira', cpf: '33344455566', jornada_id: 3, ativo: true },
        { id: 4, matricula: 'FUNC004', nome: 'Beatriz Lima', cpf: '44455566677', jornada_id: 4, ativo: true }
    ],
    statusChaves: {
        fechado: false
    },
...
async function apiRegistrarPonto(payload) {
    const { matricula, tipo_registro, ronda_em_andamento, confirm_saida_emergencia, photo_data_url, latitude, longitude } = payload;
    ...
    // 3. Rule: Zelador / Recepcionista Exit conditioned on Keys Closing
    let isSaidaEmergencia = false;
    if (tipo_registro === 'SAIDA' && jornada.exige_fechamento_chaves) {
        const chavesDevolvidas = mockStore.statusChaves.fechado;
        if (!chavesDevolvidas) {
            if (!confirm_saida_emergencia) {
                return {
                    success: false,
                    requires_keys_override: true,
                    message: "Saída bloqueada: Controle de Chaves do dia permanece EM ABERTO. Selecione Liberação Gerencial ou Saída de Emergência."
                };
            } else {
    ...
    if (tipo_registro === 'ENTRADA') {
        ...
        if (atrasoMinutos > jornada.tolerancia_entrada_minutos) {
            const isPorteiro = jornada.cargo === 'PORTEIRO';
            const tipoOcorrencia = (isPorteiro || atrasoMinutos > 15) ? 'ATRASO_CRITICO' : 'ATRASO';
            ...
            createdOcorrencia = {
                ...
                justificativa: isPorteiro ? `Atraso na Portaria (${atrasoMinutos} min excede tolerância de ${jornada.tolerancia_entrada_minutos} min)` : `Atraso de ${atrasoMinutos} min na entrada`,
    ...
    // 5. Rule: Security Guard Patrol Overtime (`HORA_EXTRA_RONDA`)
    if (tipo_registro === 'SAIDA' && jornada.cargo === 'VIGILANTE' && ronda_em_andamento) {
        const extraOcorrencia = {
            id: mockStore.ocorrencias.length + 1,
            funcionario_id: funcionario.id,
            data_referencia: dataReferencia,
            tipo_ocorrencia: 'HORA_EXTRA_RONDA',
            minutos_excedentes: 30,
            status_aprovacao: 'PENDENTE',
            justificativa: 'Ronda de segurança em andamento no encerramento do turno.',
            created_at: now.toISOString()
        };
    ...
    if (isSaidaEmergencia) {
        const emergOcorrencia = {
            ...
            status_aprovacao: 'PENDENTE',
            justificativa: 'Saída realizada com chaves em aberto por emergência. Aguardando atestado/comprovante.',
            created_at: now.toISOString()
        };
...
// GEOLOCATION INITIALIZATION (Geofencing for Security Guards)
function initGeolocation() {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            (err) => {
                if (geoText) geoText.textContent = "GPS Não Disponível: Raio Validado via IP Local (Portaria)";
            }
        );
    } else {
...
function onMatriculaChange() {
    ...
    const groupRonda = document.getElementById('group-ronda');
    ...
    if (funcionario) {
        ...
        // Display patrol checkbox for security guards (vigilantes)
        if (jornada && jornada.cargo === 'VIGILANTE') {
            groupRonda.style.display = 'block';
        } else {
            groupRonda.style.display = 'none';
        }
    } else {
...
async function handleRegistrarPonto(event, confirmEmergencia = false) {
    ...
    const rondaEmAndamento = document.getElementById('check-ronda-andamento').checked;
    ...
    try {
        const result = await window.pontoAPI.registrarPonto({
            ...
            ronda_em_andamento: rondaEmAndamento,
        ...
        if (!result.success && result.requires_keys_override) {
            openOverrideModal();
            return;
        }
        ...
        document.getElementById('group-ronda').style.display = 'none';
        document.getElementById('check-ronda-andamento').checked = false;
...
// KEYS CONTROL STATUS
function updateChavesUI() {
    const statusText = document.getElementById('keys-status-text');
    const toggleBtn = document.getElementById('btn-toggle-chaves');
    const isClosed = mockStore.statusChaves.fechado;

    if (isClosed) {
        if (statusText) statusText.innerHTML = `<strong style="color: var(--color-success)">FECHADO</strong> (Liberado para saída da zeladoria/recepção)`;
        if (toggleBtn) toggleBtn.innerHTML = `<i data-lucide="unlock"></i> Reabrir Controle de Chaves`;
    } else {
        if (statusText) statusText.innerHTML = `<strong style="color: var(--color-warning)">ABERTO</strong> (Saída travada sem devolução das chaves)`;
        if (toggleBtn) toggleBtn.innerHTML = `<i data-lucide="lock"></i> Fechar Controle de Chaves do Dia`;
    }

    if (window.lucide) window.lucide.createIcons();
}

function toggleStatusChaves() {
    mockStore.statusChaves.fechado = !mockStore.statusChaves.fechado;
    updateChavesUI();
}

// PORTEIRO DELAY ALERTS MONITORING
function updatePorteiroAlerts() {
    const banner = document.getElementById('porteiro-alerts-banner');
    const list = document.getElementById('porteiro-alerts-list');

    const porteiroOcorrencias = mockStore.ocorrencias.filter(o => {
        ...
        return j && j.cargo === 'PORTEIRO' && (o.tipo_ocorrencia === 'ATRASO' || o.tipo_ocorrencia === 'ATRASO_CRITICO');
    });
    ...
    body.innerHTML = `
        <div class="receipt-box">
            <div class="receipt-header">
                <strong>PORTARIA CONTROL LTDA</strong><br>
